...
export async function registerRoutes(app: FastifyInstance) {
  ...
  app.get('/inventory', async (request) => {
    ...
    const { tenantId, productId } = Query.parse(request.query as any);
  ...
  app.get('/cart/:id', async (request) => {
    ...
    const { id } = Params.parse(request.params as any);
  ...
  app.post('/checkout/stripe', async (request, reply) => {
    ...
    const session = await stripe.checkout.sessions.create({ mode: 'payment', success_url, cancel_url, line_items: line_items as any });
